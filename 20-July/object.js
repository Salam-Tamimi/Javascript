...
*/
class adj {
  constructor(fname,lname,age,birth,favfood,favmovie){
    this.fname = fname;
    this.lname = lname;
    this.age = age;
    this.birth = birth;
...
(
  ...
  "8-1-1999",
  ["Dawali","Pasta","Zarb"],
  ['Dark home', 'Inception', 'The Dark Knight', 'Interstellar', 'The Lord of the Rings']
  );
console.log(myadj)
...
*/
const fname=()=> {
  return persons.map(x => x.name.first);
}

const fnamesArr = fname(persons);
...
const ages = persons.reduce (function(x,y){
  return x+y.age
}, 0)
...
const maxNumber = persons.reduce (function(x,y){
  if (x.age > y.age ? x : y);
    return fullName = `${y.name.first} ${y.name.last}`;
}, 0);
...
*/
const longestName = persons.reduce (function(x,y){
